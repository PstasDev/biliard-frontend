    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          disabled={disabled || !activePlayerId}
          variant="outline"
          className="h-12 font-bold text-base touch-manipulation"
        >
          ⟲ Váltás
        </Button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2 border-t">
        <Button
          ...
          disabled={disabled}
          className={cn(
            "h-14 font-bold text-base touch-manipulation flex flex-col gap-1",
            "bg-blue-600 hover:bg-blue-700"
          )}
        >
          <span className="text-xl">🏆</span>
          <span className="text-xs truncate">{player1Name.split(' ')[0]}</span>
        </Button>
        <Button
          onClick={() => onEndFrame(player2Id)}
          disabled={disabled}
        ...
        >
          <span className="text-xl">🏆</span>
          <span className="text-xs truncate">{player2Name.split(' ')[0]}</span>
        </Button>
      </div>
    </div>
  );
